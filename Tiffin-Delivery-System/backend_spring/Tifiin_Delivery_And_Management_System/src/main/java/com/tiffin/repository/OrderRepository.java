...
public interface OrderRepository extends JpaRepository<Order, Long> {
	...
	List<Order> findAllDeliveredOrder(User u, OrderStatus delivered);
	 ...
	 //DB EXTRA-4
	 @Query("SELECT o FROM Order o WHERE o.deliveryBoy.id = :deliveryBoyId AND o.status = 'PLACED'")
	 List<Order> findAllAssignedOrders(@Param("deliveryBoyId") Long deliveryBoyId);
