 ...
 const onLogin = async () => {
  ...
  if (email.length === 0) {
   ...
   try {
    ...
    console.log(result);
    if (result.message === "Successful Auth!") {
     const { jwt, email, role, id, name, isVerified } = result;
     sessionStorage.setItem("token", jwt);
     sessionStorage.setItem("email", email);
     sessionStorage.setItem("role", role);
     ...
     sessionStorage.setItem("isVerified", isVerified); //Modified
     ...
     if (isVerified === true) {
      ...
      toast.info("Your account will be verified shortly. Thank you");
     }
     ...
     //    navigate("/vendorhomepage");
     //   } else {
     //    toast.error("Your account will be verified shortly. Thank you.");
     //   }
     //  } else if (role === "ROLE_DELIVERY_BOY") {
     //   if (isVerified === 1) {
     //    navigate("/deliveryhome");
