...
const VendorDetail = () => {
 ...
 useEffect(() => {
  const getVendorMenu = async () => {
   if (email) {
    dispatch(setVendorEmail(email)); // Store vendor email and id in Redux
    ...
    if (result.status === "error") {
     ...
     setMenuItems(result);
     console.log("Our menu items" + result);
    }
   } else {
    setError({ message: "No vendor email provided" });
   }
 ...
 return (
  <div className="vendor-detail">
   ...
   <div className="menu-list">
    {menuItems.map(
     (menu) => (console.log(menu), (<MenuCard key={menu.name} menu={menu} />))
    )}
   </div>
   <div></div>
  </div>
